<template>
  <view class="calendar bgc_fff">
    ...
    <view class="weeks flex">
      <view class="week-item tac flex-fitem" v-for="week in weeks" :key="week">{{ week }} </view>
    </view>
    <view class="flex flex-fww">
      <view
        ...
        :key="index"
        style="background-color: #fff; width: 14.285%"
        class="flex-aic flexr-jsc"
      >
        <view
          class="item_box tac font_28 color_6d7278"
          :class="{
            'bgc_3c7fff color_fff': activeIndex == item.date,
            item_box_out: item.type !== 'current',
          }"
        >
...
<script setup>
  ...
  import { initDate, getCurrentDate, getPrevMonthInfo, getNextMonthInfo } from './calendar.js'
  ...
  const getPreMonth = () => {
    const { prevYear, prevMonth } = getPrevMonthInfo(dateDay.year, dateDay.month)
  ...
  const getNextMonth = () => {
    const { nextYear, nextMonth } = getNextMonthInfo(dateDay.year, dateDay.month)
...
<style lang="scss" scoped>
  ...
  .weeks {
    font-size: 30rpx;
    padding: 32rpx 0;
    border-top: 2rpx rgba(241, 241, 241, 0.8) solid;
    .week-item {
    }
  }
