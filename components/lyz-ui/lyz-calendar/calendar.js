...
export const getPrevMonthDays = (year, month) => {
  const { prevYear, prevMonth } = getPrevMonthInfo(year, month)
...
export const getMonthLastDay = (year, month) => {
  const { nextYear, nextMonth } = getNextMonthInfo(year, month)
...
export const getNextMonthDays = (year, month) => {
  ...
  for (let i = week + 1; i <= 6; i++) {
    ...
    days.push({
      type: 'next',
      isToday: false,
      date: `${nextYear}-${nextMonth < 10 ? '0' + nextMonth : nextMonth}-${
        day < 10 ? '0' + day : day
      }`,
      day,
    })
