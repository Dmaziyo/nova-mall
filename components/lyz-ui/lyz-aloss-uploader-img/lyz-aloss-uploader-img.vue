    ...
    <view class="sunui-uploader-files">
      <block v-for="(item, index) in images" :key="index">
        <view
          ...
          :class="[item.progress < 100 ? 'sunui-uploader-file-status' : '']"
        ...
        >
          ...
          <view
            ...
            v-show="upimg_move"
            ><text class="close_box">×</text></view
          >
          <view class="sunui-loader-filecontent" v-if="item.progress < 100"
            >{{ item.progress }}%</view
          >
        </view>
      </block>
      <image
        v-if="images.length <= upload_count - 1"
        class="sunui-uploader-inputbox"
        @click="chooseImage"
        :style="upload_img_wh"
        src="http://www.liwanying.top/applate-icon/upload-icon.png"
        mode=""
      ></image>
    </view>
  </view>
...
<script setup>
  import { uploadFile } from '@/js_sdk/lyz-aloss-uploader/lyz-aloss-uploader.js'
  import configData from '@/common/config.js'
  import { getOssImgName } from '@/common/util.js'
  import { ref } from 'vue'
  let images = ref([])
