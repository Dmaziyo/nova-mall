<template>
  <view style="width: 240rpx">
    <view class="flex flex-aic xlb_input_number_box b_2 bgc_f8f8f8">
      <view
        class="xlb_input_number_btn tac b_r_2"
        :class="numberVal <= 1 ? 'color_999' : 'color_333'"
        @tap="updateNum(true)"
        >-</view
      >
      <input
        ...
        type="number"
        v-model="numberVal"
        class="xlb_input_number_ipt flex-fitem tac font_28"
      />
      <view
        class="xlb_input_number_btn tac b_l_2"
        :class="numberVal >= max ? 'color_999' : 'color_333'"
        @tap="updateNum(false)"
        >+</view
      >
    </view>
...
<script setup>
  ...
  const emitFun = lyzDebounce.debounce(
    (val) => {
      emit('update:num', val)
    },
    200,
